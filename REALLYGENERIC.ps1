...
#Secure Connection
$baseUrl = Get-ChildItem Env:\PS_instance | select -ExpandProperty Value
#The Application API Key generated in BeyondInsight
$apiKey = Get-ChildItem Env:\Auth_key | select -ExpandProperty Value
#Username of BeyondInsight user granted permission to the API Key
$runAsUser = Get-ChildItem Env:\API_user| select -ExpandProperty Value
...
[System.Security.Cryptography.X509Certificates.X509Certificate2]$script:authCert = $null;
